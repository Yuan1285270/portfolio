...
Original PDF links sit with the relevant course and announce format, page count, selection and new-tab behavior. Two equal presentation previews sit below the presentation story. Writing and Data science each have a successive row: prose on the left and that course's PDF links or Tableau preview on the right. Scripting for Data has its own course row below, with a saved original salary scatter plot and a link to the full-size PNG. The narrative names the actual restaurant-inspection exercise and technology-jobs project; raw notebooks with API credentials stay local. Preview images use automatic height. The sharing PDF has 11 selected pages. All original work remains dated to 2025, without implying present-day policy or validated deployment.
...
Requested refinements (2026-09-23): the volleyball subheading uses the chapter headline scale. The career-fair row places its 20rem photo on the left and flexible prose on the right at 960px and above; compact views retain prose before the photo. Seven source-confirmed campus visits are listed by region with Chinese and English names, without inventing per-campus experiences.

Visit photography expansion (2026-09-23): campus prose now precedes an eight-photo gallery covering all seven source-confirmed universities. Stanford and Princeton lead as a paired portrait row; Penn/Harvard/MIT form a second row; UCLA/UW/UCLA form a mixed-ratio final row. Company visits show three portraits (Silicon Valley Google, Seattle exterior, bike workshop) followed by the existing cafe/Amazon landscape pair. Rows distribute width by each image’s original aspect ratio, giving aligned image edges without crop or stretching. Gallery gaps are 1.5rem; below 768px figures stack, centered at up to 24rem. All photographs link to their larger WebP in a new tab. Captions name the exact place, and alternative text describes the scene. The Harvard photograph shows a shop near campus; the tree-lined walkway belongs to UCLA. Word-embedded images follow the 90-degree display orientation in the source document; no perspective correction or free-angle rotation. This supersedes the earlier two-photo campus/company layout notes.

OwlHacks result (2026-09-23): the user-supplied 306×724 PNG is copied byte-for-byte as owlhacks-kim-tasks.png. Collaboration prose leads a centered media pair (36rem maximum, 1.5rem gap): KIM Tasks screenshot first, native 9:16 event video second. Column widths follow the assets’ aspect ratios so their heights align without crop or stretching. Below 768px they stack, with the screenshot capped at its native 306px and the film at 18rem. Screenshot links open the original PNG; captions distinguish the actual project interface from the event footage. Do not infer unverified AI functionality, efficacy, or individual ownership from the screenshot. This supersedes the earlier OwlHacks text/film side-by-side layout.

Fencing video (2026-09-23): the user-selected ScreenRecording_10-20-2025 02-00-30_1.mov is presented directly after the fencing introduction and before the unchanged team photographs. Preserve the complete 170-second recording, native 1206×644 landscape ratio, approximately 60 fps, and original audio. A full-width native player uses controls, playsInline, preload=none, no autoplay, and a frame from 80 seconds as its poster. Encode H.264/AAC with fast-start MP4 for browser playback, with no crop or geometric transforms. Its caption identifies the footage as a fencing match recording and states the length.
